...
- Q3/Q4 按风险补充但不替代 Q1/Q2；每个 Scenario 都必须显式记录 `not_required`（含理由）或 `required`（含具体评价活动）。`required` 的每项活动必须实际执行、记录证据并解决 concern；不要求无风险场景机械执行全部象限。
...
- Showcase 必须重新执行已选 Q2，并由人类把实际产品观察映射回确认的 Given/When/Then、业务数据和价值反馈；命令全绿本身不等于用户价值已经验证。
