...
5. **场景细化**：用户以有序 Given/When/Then Scenario Set 创建后续不可变 Story Revision。
6. **本地编码执行**：CodingRun 锁定精确 Story Revision；Desktop 在隔离 Git worktree 中运行受限 Agent 和质量门，由用户审查后接受或拒绝。
7. **逻辑模型编写**：在工作区定义 LogicalEntity 与 LogicalRelationship。
8. **图投影**：Diagram、DiagramNode 和 DiagramEdge 将逻辑模型投影为可视关系图；图元素不是逻辑实体本身。
9. **本地模型辅助**：Desktop AI Modeling Agent 读取远程模型，并以受限、认证的模型 command 响应用户明确请求。
10. **一致体验**：Web 是唯一前端产品；Desktop 通过 Electron 包装同一前端并消费相同 REST/HAL 语义。
