...
本文件只维护 Evidence 建模平台用户的稳定活动主干和能力地图，不包含研发管理活动。
...
| 9    | 验证价值与学习  | fresh Q2、产品观察、Q3/Q4、独立 Review、人工接受、Respond 与 next Probe |
| 10   | 编写逻辑模型    | 逻辑实体、逻辑关系、属性、行为和定义                                    |
| 11   | 构建关系图      | 图、节点、边、布局和逻辑模型引用                                        |
| 12   | 浏览与评审      | 资源浏览、模型解释、错误反馈和变更影响                                  |
| 13   | Desktop AI 建模 | 本地 Agent、受限工具、流式活动和远程模型 command                        |
| 14   | 跨运行界面使用  | Web 共享前端、Desktop repository binding 和一致 REST/HAL 语义           |
