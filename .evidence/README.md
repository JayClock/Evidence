...
`artifacts/02-domain-model/projections/` 中的 Mermaid、Glossary 和 model context 均由权威模型、候选补丁与回归场景确定性生成，可随时重建，不是第二份模型。其他 `artifacts/02-domain-model/` 内容只记录单轮候选、展开、决策与验证证据。
