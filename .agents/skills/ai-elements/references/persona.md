...
export default function App() {
  return <Persona state="listening" variant="glint" onReady={() => console.log('Animation ready')} onLoad={() => console.log('Starting to load')} onLoadError={(error) => console.error('Failed to load:', error)} onPlay={() => console.log('Animation playing')} onPause={() => console.log('Animation paused')} onStop={() => console.log('Animation stopped')} />;
}
```
