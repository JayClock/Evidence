...
describe('calculateLayout - fulfillment axis', () => {
  ...
  it('keeps request and confirmation chain aligned to the contract', () => {
    const layoutedNodes = calculateLayout(FIXTURE_NODES, FIXTURE_EDGES);
    const nodeMap = toNodeMap(layoutedNodes);
  ...
  it('lays out confirmation-connected other_evidence for every contract context', () => {
    const layoutedNodes = calculateLayout(FIXTURE_NODES, FIXTURE_EDGES);
    const nodeMap = toNodeMap(layoutedNodes);
    expectNodePosition(nodeMap, OTHER_EVIDENCE_IN_CONTEXT_ID, {
      x: 1320,
      y: 240,
    });
    expectNodePosition(nodeMap, OUT_OF_CONTEXT_OTHER_EVIDENCE_1_ID, {
      x: 1320,
      y: 240,
    });
    expectNodePosition(nodeMap, OUT_OF_CONTEXT_OTHER_EVIDENCE_2_ID, {
      x: 1320,
      y: 240,
    });
  });
  ...
  it('places other_evidence and evidence as role to the right of confirmation in order', () => {
    ...
    expectNodePosition(nodeMap, EVIDENCE_AS_ROLE_IN_CONTEXT_ID, {
      x: 1320,
      y: 360,
    });
  });
  ...
  it('keeps context size large enough when mock nodes are shifted away from origin', () => {
    const localNodes = FIXTURE_NODES.filter(
      (node) => node.id !== REQUEST_1_ID && node.id !== CONFIRMATION_1_ID,
    ).map((node) =>
