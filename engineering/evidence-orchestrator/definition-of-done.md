...
- Nest 是唯一 server track；Electron main/preload 不实现服务端业务能力。
...
- 所有查询遵守软删除和工作区边界。
- migration、Prisma schema 或 `.evidence` YAML schema 是数据模型事实来源；iteration 只记录 delta。
...
- Electron：desktop test、typecheck、lint；发布边界变化还需 package smoke。
