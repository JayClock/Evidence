...
export async function selectOrCreateGitHubIssue(
  ...
  const issues = await listOpenGitHubIssues(pi, ctx.cwd);
  if (issues.length === 0) return createGitHubIssue(pi, ctx);
  ...
  if (selected === CREATE_ISSUE_OPTION) return createGitHubIssue(pi, ctx);
