...
export function nextStepGuidance(
  ...
  if (state.loop === 'kickoff') {
    return state.kickoff_candidate
      ? '下一步：运行 /evidence-next，审核并确认、修改、拆分、延期或停止当前 Story 候选。'
      : '下一步：运行 /evidence-next，生成一张最小 Story 候选。';
  }
  if (state.loop === 'understand') {
    ...
    if (state.understand_stage === 'scenario_review') {
      return '下一步：运行 /evidence-next，选择本轮最小业务 Scenario，或继续 TQA、拆分、延期。';
    }
    if (state.modeling_stage === 'profile_review') {
      return '下一步：运行 /evidence-next，确认或调整建模对象、建模方法及是否需要修改权威模型。';
    }
    if (state.modeling_stage === 'model_review') {
      return '下一步：运行 /evidence-next，审核模型投影与统一语言，或反馈模型、Scenario、建模方法问题。';
    }
    if (state.modeling_stage === 'candidate_ready') {
      return '下一步：运行 /evidence-next，启动独立模型挑战。';
    }
    if (state.understand_stage === 'modeling') {
      return '下一步：运行 /evidence-next，将已确认 Scenario 展开到所选模型。';
    }
    return '下一步：运行 /evidence-next，继续单 Story TQA 或生成 Scenario 候选。';
  }
  if (state.loop === 'tasking') {
    return state.tasking_stage === 'desk_check'
      ? '下一步：运行 /evidence-next，执行 Desk Check；批准计划或反馈架构、流程、Scenario 缺口。'
      : '下一步：运行 /evidence-next，生成或修订可审核的测试与实施计划。';
  }
  if (state.loop === 'pair') {
  ...
  if (state.loop === 'respond') {
    return state.respond_stage === 'decision'
      ? '下一步：运行 /evidence-next，批准或要求修订知识提升与下一项 Probe。'
      : '下一步：运行 /evidence-next，形成待审核的知识提升与下一项 Probe。';
  }
