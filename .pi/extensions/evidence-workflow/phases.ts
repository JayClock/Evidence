...
export function phaseSpecificInstructions(
  ...
  switch (phase) {
    ...
    case 'architecture':
      return `- 输出上下文映射、架构风格、技术栈、模块结构、API 契约和数据模型。\n- 新增 functional-contexts.md：场景到功能上下文的映射；test-strategy.md：Q2 验收测试如何被 Q1 组件/领域测试支撑；test-doubles.md：real/fake/stub/spy/mock 的选择。\n- 优先从 engineering/evidence-workflow/test-processes/ 选择项目级、版本化 JSON 工序；目录未覆盖时才新增或修订它。当前迭代仅保存所选工序的 JSON 快照，以便审计重演。\n- 每份工序必须有 version=1、id、applies_to.runtime、applies_to.functional_contexts、至少一个 Q1 与一个 Q2 step（id/quadrant/functional_context/test_double/task）以及 quality_gates。Markdown 只能作为说明，不能替代 JSON 工序。`;
    case 'planning':
