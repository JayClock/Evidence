...
- 架构必须把每个已计划场景从 Q2 验收测试映射到功能上下文、Q1 支撑测试和选定测试替身。优先复用 `engineering/evidence-workflow/test-processes/` 的项目级工序；当场景跨 Web、Rust、Nest 或 Tauri runtime 时，明确其有序组合 `test_plan`。测试工序是有顺序、可复用且测试先行的工作指令，不是泛化任务清单；每个 JSON 工序必须有 `version=1`、`id`、`applies_to.runtime`、`applies_to.functional_contexts`、至少一个 Q1 与一个 Q2 step（含 `test_double`）和 `quality_gates`。
