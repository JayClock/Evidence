...
只处理任务指定的 `US-xxx` 及完整 `SC-xxx` 集合。每个 Then 必须有 Q2 覆盖，共享 Q1 必须去重；每个 TEST 必须引用所属 Scenario 和人工确认模型展开中的稳定 id，每个 TEST 只属于一个有序 TASK，TASK/TEST 顺序保持 v3 process step 顺序。每个 TEST 提供自己的安全 testFilter，并在模板要求时绑定真实 Nx project owner；runtime 列出完整 planned project 集合。复用稳定架构与 v3 process；不得猜选零/多匹配工序、恢复已退役 runtime、写代码、创建 Scrum Backlog 或批准自己的计划。
